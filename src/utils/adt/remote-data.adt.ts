import { constant, constFalse, Endomorphism, FunctionN, identity, Lazy, Predicate } from 'fp-ts/lib/function';
...
import { fold as foldO, getShow as getShowOption, isNone, isSome, none, Option, some } from 'fp-ts/lib/Option';
import { Either, fold as foldEither, left, right } from 'fp-ts/lib/Either';
...
//instance
export const remoteDataInstance: Monad2<URI> &
	Foldable2<URI> &
	Traversable2<URI> &
	Bifunctor2<URI> &
	Alt2<URI> &
	...
	foldMap:
		<M>(Ms: Monoid<M>) =>
		<L, A>(fa: RemoteData<L, A>, f: (a: A) => M): M =>
			isSuccess(fa) ? f(fa.value) : Ms.empty,
	...
	sequence:
		<F>(Fs: Applicative<F>) =>
		<L, A>(ta: RemoteData<L, HKT<F, A>>): HKT<F, RemoteData<L, A>> =>
			remoteDataInstance.traverse(Fs)(ta, identity),
		...
		compare: (x, y) =>
			sign(
				pipe(
					x,
					fold(
						() => pipe(y, fold(constEq, constLt, constLt, constLt)),
						() => pipe(y, fold(constGt, constEq, constLt, constLt)),
						xError =>
							pipe(
								y,
	...
	return {
		concat: (x, y) => {
			...
			return pipe(
				x,
				fold(
					() => pipe(y, fold(constY, constY, constY, constY)),
					() =>
						pipe(
							y,
							// eslint-disable-next-line no-restricted-syntax
					...
					xError =>
						pipe(
							...
							fold(
								constX,
								constX,
								yError => failure(SE.concat(xError, yError)),
								() => y,
							),
						),
					xValue =>
...
export function combine<T, L>(...list: RemoteData<L, T>[]): RemoteData<L, T[]> {
	...
	return array.sequence(remoteDataInstance)(list);
}
